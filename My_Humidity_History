...
x_realTime = [datetime.utcfromtimestamp(j/1000).strftime('%Y-%m-%d %H:%M:%S') for j in x_timeMs]
y_humidity = [i['series'][0] for i in Result if 'series' in i]

plt.plot(x_realTime, y_humidity)
